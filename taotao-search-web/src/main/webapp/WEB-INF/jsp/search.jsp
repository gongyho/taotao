...
<div  id="pagin-btm" class="pagin fr" clstag="search|keycount|search|pre-page2">
	...
	<c:choose>
		...
		<c:when test="${end > totalPages }">
			<c:set var="end" value="${totalPages }"/>
			<c:set var="end" value="${totalPages - 5 }"/>
		</c:when>
	</c:choose>	
	<c:forEach begin="${start }" end="${end }" var="i">
		<c:choose>
			...
			<c:when test="${page ne i }">
					<a href="#" onclick="search('key',${page});">${i }</a>
			</c:when>
		</c:choose>		
	</c:forEach>
	...
	<a href="search?q=&enc=utf-8&qr=&qrst=UNEXPAND&rt=1&page=2" onclick="search('key',${page});return false;">2</a>
